...
a_theta_d = [exp(-1j* pi/2 *1/4*(-2*sin(theta)-cos(theta))),exp(-1j* pi/2 *(2*sin(theta)-cos(theta))),exp(-1j* pi/2 *(-2*sin(theta)+cos(theta))),exp(-1j* pi/2 *(2*sin(theta)+cos(theta)))];
%sum(a_theta_d)
my_pattern = 1/J^4 * abs(sum(a_theta_d,2)).^2;
