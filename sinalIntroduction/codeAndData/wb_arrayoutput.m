...
direction = [-11,38];                         % row vector containing angles for which constraints hold
...
v = [sin(direction*pi/180);-cos(direction*pi/180)];
for freqIdx = 1:nfft
    steermatrix(:,:,freqIdx) = exp(-1j*2*pi*F(freqIdx)*(p*v)./c); 
end
...
steermatrix = zeros(nfft,J);
for freqIdx = 1:nfft
    steermatrix(freqIdx,:) = exp(1j*2*pi*F(freqIdx)*(p*v)./c); 
end
comp_Freq_X = Freq_X.*steermatrix;
